...
async def upload_rfp(file: UploadFile = File(...)):
    try:
        ...
        active_pipelines[pipeline_id] = {"status": "processing", "result": None}
        ...
        def run_pipeline():
            try:
                ...
                active_pipelines[pipeline_id].update(status="complete", result=output)
            except Exception as e:
                active_pipelines[pipeline_id].update(status="failed", result={"error": str(e)})
...
def get_result(pipeline_id: str):
    try:
        ...
        return {"status": data["status"], **(data["result"] or {})}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
